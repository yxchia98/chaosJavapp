...
public class CpuLoader extends Loader {
	...
	public void load() {
		...
		int numCores = Runtime.getRuntime().availableProcessors();	// get number of available cores on system
		LocalDateTime endtime = LocalDateTime.now().plusSeconds(this.duration);	// set end duration
		for (int i = 0; i < numCores; i++) {
			// instantiate BusyThread object for all cores
